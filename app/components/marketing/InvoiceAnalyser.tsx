...
export default function InvoiceAnalyser() {
  ...
  return (
    <section id="invoice-analyser" className="relative overflow-hidden py-20 md:py-32 px-5 md:px-20">
      {/* Background image — full brightness, no heavy overlay */}
      <div className="absolute inset-0 z-0">
        <img src="/invoice-bg.jpg" alt="" className="w-full h-full object-cover object-center" />
        {/* Very light scrim just to prevent blinding contrast */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <FadeInView>
          <div className="text-center mb-10 md:mb-14">
            ...
            {/* Black headings on light bg */}
            <h2 className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tighter mb-4 md:mb-6 text-black"
          ...
          {!result ? (
            <FadeInView delay={0.1}>
              ...
              <div
                ...
                style={{
                  borderColor: dragging ? "#591bff" : file ? "#f94580" : "rgba(0,0,0,0.15)",
                ...
                {file ? (
                  <div className="flex flex-col items-center gap-4">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
                      style={{ background: "rgba(249,69,128,0.12)", border: "1px solid rgba(249,69,128,0.25)" }}>
                  ...
                  <div className="flex flex-col items-center gap-4">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
                      style={{ background: "rgba(89,27,255,0.1)", border: "1px solid rgba(89,27,255,0.2)" }}>
                      <Upload className="w-8 h-8 text-[#591bff]" />
                    </div>
                    <div>
                      <p className="font-bold text-lg text-black">Drop your invoice here</p>
              ...
              {error && (
                <div className="mt-4 p-4 rounded-xl text-sm flex items-center gap-2 bg-red-50 border border-red-200 text-red-700">
              ...
              <button onClick={analyse} disabled={!file || loading}
                ...
                style={{ background: "linear-gradient(135deg, #f94580 0%, #591bff 100%)", boxShadow: "0 0 30px rgba(249,69,128,0.3), inset 0 1px 0 rgba(255,255,255,0.15)" }}>
            ...
            <FadeInView>
              <div className="space-y-4"
                style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", borderRadius: "2rem", padding: "2rem", boxShadow: "0 8px 40px rgba(0,0,0,0.1), inset 0 1px 0 rgba(255,255,255,0.8)" }}>
                {/* Saving headline */}
                <div className="rounded-[1.5rem] p-6 text-white text-center"
                ...
                <div className="flex justify-between items-center px-2 py-3">
                  <div className="text-center">
                    <p className="text-xs text-black/40 mb-1">Currently paying</p>
                    <p className="text-2xl font-black text-black">{fmt(result.totalMonthly)}</p>
                    <p className="text-xs text-black/30">{result.provider}/mo</p>
                  </div>
                  <TrendingDown className="w-7 h-7 text-[#f94580]" />
                  <div className="text-center">
                    <p className="text-xs text-black/40 mb-1">With ITC</p>
                    <p className="text-2xl font-black text-[#591bff]">{fmt(result.itcTotalMonthly)}</p>
                    <p className="text-xs text-black/30">per month</p>
                  </div>
                </div>

                {result.services.length > 0 && (
                  <div className="border-t border-black/10 pt-4 space-y-2">
                    <p className="text-xs font-bold uppercase tracking-widest text-black/30 mb-3">Services</p>
                    {result.services.map((s, i) => (
                      <div key={i} className="flex justify-between items-center py-2 border-b border-black/8 last:border-0">
                        <div>
                ...
                {result.recommendations.length > 0 && (
                  <div className="space-y-2 pt-2">
                    <p className="text-xs font-bold uppercase tracking-widest text-black/30 mb-3">Recommendations</p>
                    {result.recommendations.map((r, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <CheckCircle2 className="w-4 h-4 text-[#591bff] shrink-0 mt-0.5" />
                        <p className="text-sm text-black/70">{r}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  ...
                  <button onClick={() => { setResult(null); setFile(null) }}
                    className="px-6 py-4 rounded-full font-bold text-sm text-black/60 border border-black/15 hover:bg-black/5 transition-colors">
                    Analyse Another
                  </button>
                </div>
              </div>
